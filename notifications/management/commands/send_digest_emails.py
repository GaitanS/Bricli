...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--force',
            action='store_true',
            help='Force send digest even if not the right time'
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Starting {period} digest email sending (dry_run={dry_run})')
        )
        ...
        if not force:
            if period == 'daily' and now.hour != 8:  # Send daily digests at 8 AM
                self.stdout.write(
                    self.style.WARNING('Not the right time for daily digests (should be 8 AM)')
                )
                return
            
            elif period == 'weekly' and (now.weekday() != 0 or now.hour != 8):  # Monday 8 AM
                self.stdout.write(
                    self.style.WARNING('Not the right time for weekly digests (should be Monday 8 AM)')
                )
                return
        ...
        if user_id:
            ...
            users = User.objects.filter(
                is_active=True,
                notificationpreference__digest_frequency=period
            ).select_related('notificationpreference')
        ...
        for user in users:
            try:
                ...
                if dry_run:
                    self.stdout.write(
                        self.style.WARNING(f'Would send {period} digest to {user.email}')
                    )
                    sent_count += 1
                else:
                    ...
                    if success:
                        self.stdout.write(
                            self.style.SUCCESS(f'Sent {period} digest to {user.email}')
                        )
                        sent_count += 1
                    else:
                        self.stdout.write(
                            self.style.ERROR(f'Failed to send digest to {user.email}')
                        )
                        error_count += 1
        ...
        if not dry_run:
            self.stdout.write(
                self.style.SUCCESS('Digest sending completed!')
            )
        else:
            self.stdout.write(
                self.style.WARNING('Dry run completed. Use --no-dry-run to actually send emails.')
            )