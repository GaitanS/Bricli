...
from notifications.services import NotificationService, PushNotificationService
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--expired-only',
            action='store_true',
            help='Only delete notifications that have an explicit expiry date'
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Starting notification cleanup (dry_run={dry_run})')
        )
        ...
        if expired_count > 0:
            ...
            if not dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'Deleted {deleted_count} notifications')
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f'Would delete {expired_count} notifications')
                )
        else:
        ...
        if inactive_count > 0:
            ...
            if not dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'Deleted {deleted_subs} push subscriptions')
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f'Would delete {inactive_count} push subscriptions')
                )
        else:
        ...
        if not dry_run:
            ...
            self.stdout.write(
                self.style.WARNING('Dry run completed. Use --no-dry-run to actually delete.')
            )