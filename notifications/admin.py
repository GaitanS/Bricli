...
class NotificationAdmin(admin.ModelAdmin):
    list_display = ["title", "recipient", "notification_type", "priority", "is_read", "created_at", "action_buttons"]
    list_filter = ["notification_type", "priority", "is_read", "created_at", "email_sent", "push_sent"]
    search_fields = ["title", "message", "recipient__username", "recipient__email"]
    readonly_fields = ["created_at", "read_at"]
    date_hierarchy = "created_at"

    fieldsets = (
        ("Informații de bază", {"fields": ("recipient", "sender", "notification_type", "priority")}),
        ("Conținut", {"fields": ("title", "message", "action_url")}),
        ("Obiect asociat", {"fields": ("related_object_type", "related_object_id"), "classes": ("collapse",)}),
        ("Status", {"fields": ("is_read", "read_at", "expires_at")}),
        ("Livrare", {"fields": ("email_sent", "push_sent"), "classes": ("collapse",)}),
    ...
    def action_buttons(self, obj):
        ...
        if obj.action_url:
            buttons.append(f'<a class="button" href="{obj.action_url}" target="_blank">Vezi acțiunea</a>')
    ...
    def mark_as_unread(self, request, queryset):
        ...
        self.message_user(request, f"{updated} notificări au fost marcate ca necitite.")
    ...
    def delete_expired(self, request, queryset):
        ...
        now = timezone.now()
        expired_count = queryset.filter(expires_at__lt=now).delete()[0]

        self.message_user(request, f"{expired_count} notificări expirate au fost șterse.")
...
class NotificationPreferenceAdmin(admin.ModelAdmin):
    list_display = ["user", "digest_frequency", "email_enabled", "push_enabled", "quiet_hours", "updated_at"]
    list_filter = ["digest_frequency", "email_new_orders", "push_new_orders", "created_at", "updated_at"]
    ...
    fieldsets = (
        ...
        ("Setări generale", {"fields": ("digest_frequency", "quiet_hours_start", "quiet_hours_end")}),
...
class PushSubscriptionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Utilizator", {"fields": ("user", "is_active")}),
        ("Detalii abonament", {"fields": ("endpoint", "p256dh_key", "auth_key", "user_agent")}),
    ...
    def activate_subscriptions(self, request, queryset):
        ...
        self.message_user(request, f"{updated} abonamente au fost activate cu succes.")
    ...
    def deactivate_subscriptions(self, request, queryset):
        ...
        self.message_user(request, f"{updated} abonamente au fost dezactivate cu succes.")
    ...
    def test_push_notifications(self, request, queryset):
        ...
        for subscription in queryset.filter(is_active=True):
            try:
                ...
                error_count += 1
                self.message_user(
                    request, f"Eroare la trimiterea notificării pentru {subscription.user}: {str(e)}", level="ERROR"
                )

        if success_count > 0:
            self.message_user(request, f"{success_count} notificări de test trimise cu succes.")

        if error_count > 0:
            self.message_user(
                request, f"{error_count} notificări nu au putut fi trimise din cauza erorilor.", level="WARNING"
            )
