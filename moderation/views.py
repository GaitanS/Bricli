...
def create_report_view(request):
    ...
    try:
        ...
        report = create_report(
            reporter=request.user, content_object=content_object, report_type=report_type, description=description
        )

        messages.success(request, "Raportarea a fost trimisă cu succes. Echipa noastră va analiza situația.")
...
def quick_report(request, content_type_id, object_id, report_type):
    ...
    try:
        ...
        if existing_report:
            return JsonResponse({"success": False, "message": "Ai raportat deja acest conținut."})
        ...
        return JsonResponse({"success": True, "message": "Conținutul a fost raportat cu succes."})

    except (ContentType.DoesNotExist, ValueError):
        return JsonResponse({"success": False, "message": "A apărut o eroare la procesarea raportării."})
...
def block_user_request(request, user_id):
    ...
    try:
        ...
        messages.success(request, "Cererea de blocare a fost trimisă către echipa de moderare.")
...
def test_rate_limit(request):
    ...
    return render(
        request, "moderation/test_rate_limit.html", {"limit_types": [choice[0] for choice in Report.REPORT_TYPES]}
    )
