...
from .models import ImageModerationQueue, IPBlock, ModerationAction, RateLimit, Report
...
class IPBlockAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("ip_address", "reason", "is_permanent")}),
        ("Detalii blocare", {"fields": ("blocked_until", "user_agent", "notes")}),
        ("Moderare", {"fields": ("blocked_by", "blocked_at")}),
    )
...
class ReportAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("reporter", "report_type", "status")}),
        ("Conținut raportat", {"fields": ("content_type", "object_id", "description")}),
        ("Moderare", {"fields": ("reviewed_by", "resolution_notes")}),
    ...
    def mark_as_resolved(self, request, queryset):
        updated = queryset.update(status="resolved", reviewed_by=request.user, updated_at=timezone.now())
    ...
    def mark_as_dismissed(self, request, queryset):
        updated = queryset.update(status="dismissed", reviewed_by=request.user, updated_at=timezone.now())
...
class ModerationActionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("moderator", "target_user", "action_type")}),
        ("Detalii", {"fields": ("reason", "duration_hours", "expires_at")}),
        ("Conținut (opțional)", {"fields": ("content_type", "object_id")}),
...
class ImageModerationQueueAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("status", "image_path")}),
        ("Conținut", {"fields": ("content_type", "object_id")}),
        ("Analiză automată", {"fields": ("has_faces", "has_text", "confidence_score")}),
        ("Moderare manuală", {"fields": ("reviewed_by", "review_notes", "reviewed_at")}),
