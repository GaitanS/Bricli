...
class PaymentIntent(models.Model):
    ...
    def mark_as_succeeded(self):
        ...
        if self.status != "succeeded":
            ...
            wallet, created = CreditWallet.objects.get_or_create(user=self.user, defaults={"balance_cents": 0})

            wallet.add_amount(
                amount_cents=self.amount_cents,
                reason="top_up",
                meta={"payment_intent_id": self.stripe_payment_intent_id, "payment_method": "stripe"},
            )
...
class StripeCustomer(models.Model):
    ...
    def get_or_create_for_user(cls, user):
        ...
        try:
            ...
            return cls.objects.create(user=user, stripe_customer_id=stripe_customer.id)
