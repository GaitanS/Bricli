...
from services.lead_fee_service import InsufficientBalanceError, LeadFeeService
from services.models import CreditWallet, Order, Shortlist, WalletTransaction
...
class TestWalletOperations:
    ...
    def user(self):
        ...
        return User.objects.create_user(
            username="testuser", email="test@example.com", password="testpass123", user_type="craftsman"
        )
    ...
    def test_wallet_top_up(self, wallet):
        ...
        transaction = wallet.add_amount(amount_cents=5000, reason="top_up", meta={"payment_method": "stripe"})
    ...
    def test_wallet_deduct_success(self, wallet):
        ...
        transaction = wallet.deduct_amount(amount_cents=2000, reason="lead_fee", meta={"order_id": 1})
    ...
    def test_wallet_deduct_insufficient_balance(self, wallet):
        ...
        with pytest.raises(ValueError, match="Sold insuficient"):
            wallet.deduct_amount(amount_cents=2000, reason="lead_fee", meta={})
...
class TestLeadFeeService:
    ...
    def client_user(self):
        ...
        return User.objects.create_user(
            username="client1", email="client@example.com", password="pass123", user_type="client"
        )
    ...
    def craftsman_user(self):
        ...
        user = User.objects.create_user(
            username="craftsman1", email="craftsman@example.com", password="pass123", user_type="craftsman"
        )
        ...
        CraftsmanProfile.objects.create(user=user, display_name="Test Craftsman", county=county, city=city)
        return user
    ...
    def test_charge_shortlist_fee_sufficient_balance(self, order, craftsman_user):
        ...
        transaction = WalletTransaction.objects.filter(wallet=wallet, reason="lead_fee").first()
    ...
    def test_charge_shortlist_fee_insufficient_balance(self, order, craftsman_user):
        ...
        with pytest.raises(InsufficientBalanceError) as exc_info:
            LeadFeeService.charge_shortlist_fee(order=order, craftsman_user=craftsman_user)
    ...
    def test_can_afford_shortlist(self, order, craftsman_user):
        ...
        can_afford, balance, fee = LeadFeeService.can_afford_shortlist(craftsman_user, order)
