...
from .payment_dummy import get_dummy_provider, is_dummy_mode
from .payment_models import PaymentIntent, PaymentMethod, StripeCustomer

logger = logging.getLogger(__name__)

# Set Stripe API key (only if not in dummy mode)
if not is_dummy_mode():
    stripe.api_key = settings.STRIPE_SECRET_KEY
else:
    logger.info("🔧 Stripe disabled - using DummyPaymentProvider for local development")


class CreatePaymentIntentView(LoginRequiredMixin, View):
    """Create a Stripe payment intent for wallet top-up (or dummy for local dev)"""
    ...
    def post(self, request):
        try:
            ...
            if amount_cents < 5000 or amount_cents > 500000:
                return JsonResponse({"error": "Suma trebuie să fie între 50 și 5000 RON"}, status=400)

            # DUMMY MODE: Use mock payment provider
            if is_dummy_mode():
                dummy_provider = get_dummy_provider()
                intent = dummy_provider.create_payment_intent(
                    amount=amount_cents,
                    currency="ron",
                    metadata={"user_id": request.user.id, "purpose": "wallet_topup"},
                )

                # Auto-succeed the payment and credit wallet immediately (dev convenience)
                with transaction.atomic():
                    wallet, _ = CreditWallet.objects.get_or_create(user=request.user, defaults={"balance_cents": 0})
                    wallet.add_amount(
                        amount_cents=amount_cents,
                        reason="top_up",
                        meta={"payment_method": "dummy", "dummy_intent_id": intent.id},
                    )

                messages.success(request, f"🔧 Dummy Payment: {amount_cents/100:.2f} RON adăugați în wallet (dev mode)")
            ...
            return JsonResponse(
                {"client_secret": intent.client_secret, "payment_intent_id": payment_intent.id, "dummy_mode": False}
            )

        except Exception as e:
            logger.error(f"Error creating payment intent: {str(e)}")
            return JsonResponse({"error": "A apărut o eroare la procesarea plății"}, status=500)
        ...
        if payment_intent_id:
            try:
                ...
                payment_intent = PaymentIntent.objects.get(
                    stripe_payment_intent_id=payment_intent_id, user=self.request.user
                )

                if intent.status == "succeeded" and payment_intent.status != "succeeded":
                    payment_intent.mark_as_succeeded()
                    messages.success(
                        self.request, f"Plata de {payment_intent.amount_lei} RON a fost procesată cu succes!"
                    )
...
class StripeWebhookView(View):
    ...
    def _handle_payment_succeeded(self, stripe_payment_intent):
        ...
        try:
            payment_intent = PaymentIntent.objects.get(stripe_payment_intent_id=stripe_payment_intent["id"])
...
class PaymentMethodsView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["payment_methods"] = PaymentMethod.objects.filter(user=self.request.user, is_active=True)
        return context
...
class WalletTopUpView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Get user's wallet
        wallet, created = CreditWallet.objects.get_or_create(user=self.request.user, defaults={"balance_cents": 0})

        context["wallet"] = wallet
        context["dummy_mode"] = is_dummy_mode()

        # Only include Stripe key if not in dummy mode
        if not is_dummy_mode():
