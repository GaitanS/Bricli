...
def can_post_services(user):
    ...
    """
    return user.is_authenticated and user.user_type == "craftsman" and hasattr(user, "craftsman_profile")
...
def can_accept_quote(user, quote):
    ...
    """
    return user.is_authenticated and quote.order.client == user and quote.status == "pending"
...
def can_review_order(user, order):
    ...
    """
    return (
        user.is_authenticated and order.client == user and order.status == "completed" and not hasattr(order, "review")
    )
