...
from accounts.services import ProfileCompletionService, BadgeService
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not options['badges_only']:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Actualizate {updated_profiles} profiluri')
            )
        ...
        if not options['completion_only']:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Actualizate badge-uri pentru {updated_badges} profiluri')
            )
