...
from .models import City, County, CraftsmanPortfolio, CraftsmanProfile, User
...
class UserRegistrationForm(UserCreationForm):
    email = forms.EmailField(
        required=True, widget=forms.EmailInput(attrs={"placeholder": "adresa@email.com", "autocomplete": "email"})
    )
    ...
    last_name = forms.CharField(
        ...
        required=True,
        widget=forms.TextInput(
            attrs={"placeholder": "Nume", "pattern": r"[A-Za-zÀ-ÿ\s]+", "title": "Doar litere și spații sunt permise"}
        ),
    )
    phone_number = forms.CharField(
        max_length=15,
        required=False,
        widget=forms.TextInput(attrs={"type": "tel", "pattern": r"[0-9+\-\s\(\)]+", "placeholder": "ex. 0721234567"}),
    )
...
class SimpleUserRegistrationForm(forms.ModelForm):
    ...
    first_name = forms.CharField(
        max_length=30,
        required=True,
        widget=forms.TextInput(attrs={"class": "form-control form-control-lg", "placeholder": "Prenume"}),
        label="Prenume",
    )
    ...
    last_name = forms.CharField(
        max_length=30,
        required=True,
        widget=forms.TextInput(attrs={"class": "form-control form-control-lg", "placeholder": "Nume de familie"}),
        label="Nume de familie",
    )

    email = forms.EmailField(
        required=True,
        widget=forms.EmailInput(attrs={"class": "form-control form-control-lg", "placeholder": "adresa@email.com"}),
    ...
    phone_number = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control form-control-lg", "placeholder": "0721234567 (opțional)", "type": "tel"}
        ),
    ...
    password = forms.CharField(
        widget=forms.PasswordInput(
            attrs={"class": "form-control form-control-lg", "placeholder": "Parola (min. 8 caractere)"}
        ),
        label="Parola",
    )

    password_confirm = forms.CharField(
        widget=forms.PasswordInput(attrs={"class": "form-control form-control-lg", "placeholder": "Confirmă parola"}),
...
class SimpleCraftsmanRegistrationForm(forms.ModelForm):
    ...
    phone_number = forms.CharField(
        ...
        required=True,
        widget=forms.TextInput(
            attrs={"class": "form-control form-control-lg", "placeholder": "0721234567", "type": "tel"}
        ),
    ...
    company_name = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control form-control-lg", "placeholder": "Numele companiei (opțional)"}
        ),
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            for service in self.cleaned_data["services"]:
                CraftsmanService.objects.create(craftsman=craftsman_profile, service=service)
...
class ProfileUpdateForm(forms.ModelForm):
    # Craftsman profile fields
    display_name = forms.CharField(
        max_length=100, required=False, widget=forms.TextInput(attrs={"class": "form-control"}), label="Nume afișat"
    )
    county = forms.ModelChoiceField(
        queryset=County.objects.all(),
        required=False,
        empty_label="Selectează județul",
    ...
    city = forms.ModelChoiceField(
        ...
        required=False,
        empty_label="Selectează orașul",
        widget=forms.Select(attrs={"class": "form-control"}),
        label="Orașul",
    )
    coverage_radius_km = forms.IntegerField(
        required=False,
        widget=forms.NumberInput(attrs={"class": "form-control", "min": "5", "max": "150", "placeholder": "ex. 25"}),
        label="Raza de acoperire (km)",
    )
    bio = forms.CharField(
        required=False,
        widget=forms.Textarea(
            attrs={"class": "form-control", "rows": 4, "placeholder": "Descrie-ți serviciile și experiența..."}
        ),
        label="Descrierea ta profesională",
    )
    years_experience = forms.IntegerField(
        required=False,
        widget=forms.NumberInput(
            attrs={"class": "form-control", "type": "number", "min": "0", "max": "50", "placeholder": "ex. 5"}
        ),
    ...
    hourly_rate = forms.DecimalField(
        ...
        label="Tarif orientativ pe oră (RON)",
    )
    min_job_value = forms.DecimalField(
        required=False,
        max_digits=8,
        decimal_places=2,
        widget=forms.NumberInput(
        ...
        label="Valoarea minimă lucrare (RON)",
    )
    company_cui = forms.CharField(
        max_length=20,
        required=False,
    ...
    facebook_url = forms.URLField(
        required=False,
        widget=forms.URLInput(attrs={"class": "form-control", "placeholder": "https://facebook.com/pagina-ta"}),
        label="Facebook",
    )
    instagram_url = forms.URLField(
        required=False,
        widget=forms.URLInput(attrs={"class": "form-control", "placeholder": "https://instagram.com/contul-tau"}),
        label="Instagram",
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["profile_picture"].widget = forms.FileInput(
            attrs={"class": "form-control", "style": "display: none;", "accept": "image/*"}
        )
        ...
        # If user is craftsman, populate craftsman fields
        if hasattr(self.instance, "craftsman_profile"):
            craftsman = self.instance.craftsman_profile
            self.fields["display_name"].initial = craftsman.display_name
            self.fields["county"].initial = craftsman.county
            self.fields["city"].initial = craftsman.city
            self.fields["coverage_radius_km"].initial = craftsman.coverage_radius_km
            self.fields["bio"].initial = craftsman.bio
            self.fields["years_experience"].initial = craftsman.years_experience
...
class CraftsmanProfileForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["display_name"].help_text = (
            "Numele sub care vei apărea pe platformă (nume persoană sau denumire comercială)"
        )
        ...
        self.fields["years_experience"].widget.attrs.update(
            {"type": "number", "min": "0", "max": "50", "placeholder": "ex. 5"}
        )
        self.fields["hourly_rate"].widget.attrs.update(
            {"type": "number", "min": "10", "max": "1000", "step": "0.01", "placeholder": "ex. 50.00"}
        )
        self.fields["min_job_value"].widget.attrs.update(
            {"type": "number", "min": "50", "max": "100000", "step": "0.01", "placeholder": "ex. 200.00"}
        )
        self.fields["coverage_radius_km"].widget.attrs.update(
            {"type": "number", "min": "5", "max": "150", "placeholder": "ex. 25"}
        )

        # Add validation for text fields
        self.fields["display_name"].widget.attrs.update(
            {"maxlength": "100", "placeholder": "ex. Ion Popescu sau SC Construct SRL"}
        )
        self.fields["company_cui"].widget.attrs.update({"maxlength": "20", "placeholder": "ex. RO12345678"})

        # URL fields
        self.fields["website_url"].widget.attrs.update({"placeholder": "https://www.site-ul-tau.ro"})
        self.fields["facebook_url"].widget.attrs.update({"placeholder": "https://facebook.com/pagina-ta"})
        self.fields["instagram_url"].widget.attrs.update({"placeholder": "https://instagram.com/contul-tau"})
...
class CraftsmanPortfolioForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["image"].widget.attrs.update({"class": "form-control", "accept": "image/*", "required": True})
        ...
        self.fields["description"].widget.attrs.update(
            {"class": "form-control", "rows": 3, "placeholder": "Descrie lucrarea realizată...", "maxlength": "500"}
        )
        ...
        self.fields["description"].widget = forms.Textarea(
            attrs={"rows": 3, "class": "form-control", "placeholder": "Descrie lucrarea realizată..."}
        )
...
class CraftsmanSkillsForm(forms.Form):
    ...
    primary_skill = forms.ChoiceField(
        choices=SKILL_CHOICES, label="Meseria principală", widget=forms.Select(attrs={"class": "form-select"})
    )
    ...
    def clean_email(self):
        ...
        if email:
            ...
            if not User.objects.filter(email=email, is_active=True).exists():
                raise ValidationError("Nu există niciun cont activ cu această adresă de email.")
        return email
