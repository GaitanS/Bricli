...
def validate_no_profanity(value):
    ...
    for word in PROFANITY_WORDS:
        ...
        if re.search(pattern, clean_value):
            raise ValidationError("Textul conține limbaj nepotrivit. Te rugăm să folosești un limbaj respectuos.")
...
def validate_romanian_phone(value):
    ...
    if not is_valid:
        raise ValidationError(
            "Numărul de telefon nu este valid. " "Folosește format românesc: 0721234567 sau +40721234567"
        )
...
def validate_bio_length(bio):
    ...
    if len(clean_bio) < 200:
        raise ValidationError(
            f"Biografia trebuie să aibă cel puțin 200 caractere. Ai scris {len(clean_bio)} caractere."
        )
