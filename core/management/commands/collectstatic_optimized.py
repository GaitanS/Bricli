...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--no-post-process',
            action='store_false', dest='post_process',
    ...
    def minify_files(self):
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(
                    'cssmin and jsmin packages are required for minification. '
                    'Install with: pip install cssmin jsmin'
                )
            )
            return
        ...
        if not static_root or not os.path.exists(static_root):
            self.stdout.write(
                self.style.ERROR('STATIC_ROOT not found. Run collectstatic first.')
            )
            return
            ...
            for file in files:
                ...
                if file.endswith('.css') and not file.endswith('.min.css'):
                    try:
                        ...
                        self.stdout.write(
                            self.style.WARNING(f'Failed to minify {file}: {e}')
                        )
            ...
            for file in files:
                ...
                if any(file.endswith(ext) for ext in extensions_to_gzip):
                    try:
                        ...
                        self.stdout.write(
                            self.style.WARNING(f'Failed to gzip {file}: {e}')
                        )